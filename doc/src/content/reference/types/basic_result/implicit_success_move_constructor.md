...
Note that if `T = void`, `value_type` will be default constructed.  Calls {{% api "hook_result_move_construction(basic_result<T, E, NoValuePolicy> *, U &&)" %}} with `this` and `success_type<T> &&`.
