...
template<class T, class E> using result = OUTCOME_V2_NAMESPACE::experimental::erased_result<T, E>;
...
protected:
  ...
  virtual bool _do_equivalent(const SYSTEM_ERROR2_NAMESPACE::status_code<void> &, const SYSTEM_ERROR2_NAMESPACE::status_code<void> &) const noexcept override final
  {
    return false;
  }
  virtual SYSTEM_ERROR2_NAMESPACE::generic_code _generic_code(const SYSTEM_ERROR2_NAMESPACE::status_code<void> &) const noexcept override final
  {
    return {};
  }
  ...
  SYSTEM_ERROR2_NORETURN virtual void _do_throw_exception(const SYSTEM_ERROR2_NAMESPACE::status_code<void> &) const override final
  {
    abort();
  }
};
