...
BOOST_AUTO_TEST_CASE(works / success - failure, "Tests that the success and failure type sugars work as intended")
{
...
#endif
  {
    ...
    static_assert(std::is_same<decltype(a.value), int>::value, "");
    // static_assert(std::is_same<decltype(b.value), int>::value, "");
    static_assert(std::is_same<decltype(c.value), const char *>::value, "");
  }
  {
    ...
    static_assert(std::is_same<decltype(a.error), int>::value, "");
