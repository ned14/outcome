...
#define OUTCOME_COROUTINE_NAMESPACE_BEGIN                                                                                                                                                                                                                                                                                      \
  namespace std                                                                                                                                                                                                                                                                                                                \
  {
#define OUTCOME_COROUTINE_NAMESPACE_END }
...
namespace detail
{
  ...
  template <class Awaitable, bool suspend_initial, bool is_void> struct outcome_promise_type
  {
    using container_type = typename Awaitable::container_type;
    using result_set_type = std::conditional_t<suspend_initial, std::atomic<bool>, fake_atomic<bool>>;
    union {
      detail::empty_type _default{};
  ...
  template <class Awaitable, bool suspend_initial> struct outcome_promise_type<Awaitable, suspend_initial, true>
  {
  ...
  template <class Awaitable, bool suspend_initial> constexpr inline auto move_result_from_promise_if_not_void(outcome_promise_type<Awaitable, suspend_initial, false> &p) { return static_cast<typename Awaitable::container_type &&>(p.result); }
  template <class Awaitable, bool suspend_initial> constexpr inline void move_result_from_promise_if_not_void(outcome_promise_type<Awaitable, suspend_initial, true> & /*unused*/) {}
}  // namespace detail

/*! AWAITING HUGO JSON CONVERSION TOOL
SIGNATURE NOT RECOGNISED
*/
template <class Cont> struct OUTCOME_NODISCARD awaitable
{
  using container_type = Cont;
  using promise_type = detail::outcome_promise_type<awaitable, false, std::is_void<container_type>::value>;
...
SIGNATURE NOT RECOGNISED
*/
template <class Cont> struct OUTCOME_NODISCARD task
{
  using container_type = Cont;
  using promise_type = detail::outcome_promise_type<task, true, std::is_void<container_type>::value>;
  coroutine_handle<promise_type> _h;
  ...
  bool await_ready() { return _h.promise().result_set.load(std::memory_order_acquire); }
  container_type await_resume()
  {
    assert(_h.promise().result_set.load(std::memory_order_acquire));
    if(!_h.promise().result_set.load(std::memory_order_acquire))
