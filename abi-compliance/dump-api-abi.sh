...
rm -f $DUMPFILE.xml
echo "<version>$DUMPFILE-gcc7-cxx14</version><headers>../single-header/abi.hpp</headers><libs>build-gcc7-cxx14/liboutcome-abi-lib-gcc7-cxx14.so</libs>" > $DUMPFILE.xml
RETCODE=0
abi-compliance-checker -l Outcome -dump $DUMPFILE.xml -gcc-path g++-7 || RETCODE=1
echo "<version>$DUMPFILE-gcc9-cxx17</version><headers>../single-header/abi.hpp</headers><libs>build-gcc9-cxx17/liboutcome-abi-lib-gcc9-cxx17.so</libs>" > $DUMPFILE.xml
